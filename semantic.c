...
//      exit(4);
//    } else if (node->symbol->type != AST_VAR && node->symbol->type != SYMBOL_PARAM && node->symbol->type != SYMBOL_POINT){
//      fprintf(stderr,"Variable %s not declared.%d\n", node->symbol->yytext,node->symbol->type);
//      exit(4);
//    }
//  }
//
...
//    if (node->symbol->type == TK_IDENTIFIER || node->symbol->type == SYMBOL_PARAM || node->symbol->type == AST_VAR) {
...
//      exit(4);
//    } else if (node->symbol->type == AST_VAR) {
//      fprintf(stderr,"Scalar %s being used as function type.\n", node->symbol->yytext);
//      exit(4);
//    } else if (node->symbol->type == SYMBOL_POINT) {
